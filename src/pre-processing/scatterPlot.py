...
dataset1 = pd.read_csv('data/original/percentuale_centraline_limite_PM10_superato_piu_di_35_giorni.csv')
dataset2 = pd.read_csv('data/original/percentuale_energia_solare_comunale.csv')
dataset3 = pd.read_csv('data/original/percentuale_veicoli_basse_emissioni.csv')
dataset4 = pd.read_csv('data/original/percentuale_verde_urbano.csv')
...
dataset2['Percentuale_comune'] = dataset2['Percentuale_comune'].replace(" ", "").replace(" - ", "0").replace(",,", ",0,").astype(float)
dataset2['Percentuale_partecipate'] = dataset2['Percentuale_partecipate'].replace(" ", "").replace(" - ", "0").replace(",,", ",0,").astype(float)
dataset2.set_index('City')
dataset2['Percentuale_energia_solare'] = dataset2[['Percentuale_partecipate', 'Percentuale_comune']].sum(axis=1).round(2)
...
merged_df = pd.merge(merged_df, dataset2[['City','Percentuale_energia_solare']], on='City', how= 'outer').set_index('City')
